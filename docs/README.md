...
Guides for integrating **`weg-shared-layout`** (`<weg-header>`, `<weg-footer>`) into host applications.
...
| **[../demo/angular16/README.md](../demo/angular16/README.md)** | Runnable Angular 16 demo (`pnpm run demo:angular16`) |
...
For newer Angular with signals, see [weg-angular-demo](https://github.com/jobsac/weg-angular-demo).
...
| `weg-shared-layout/hydrate` | Stencil hydrate build for SSR pipelines that render custom elements server-side |
| `weg-shared-layout` | Root package typings plus the small `format()` helper |
...
| Script | Action |
| --- | --- |
| `pnpm run build` | Stencil build → `dist/`, `loader/`, `hydrate/` |
| `pnpm run demo:angular16` | Start the Angular 16 demo from `demo/angular16` |
| `pnpm run demo:angular16:build` | Production-build the Angular 16 demo |
| `pnpm run demo:angular16:install` | Install demo dependencies in `demo/angular16` |
